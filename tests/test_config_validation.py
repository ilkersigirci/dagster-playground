"""From https://github.com/dagster-io/dagster/pull/12536"""
...
def test_validators_basic() -> None:
    ...
    assert greet_user_job.execute_in_process(
        {
            "ops": {
                "greet_user": {
                    "config": {"name": "Arthur Miller", "username": "arthur"}
                }
            }
        }
    ).success
    ...
    with pytest.raises(ValidationError, match="must contain a space"):
        greet_user_job.execute_in_process(
            {
                "ops": {
                    "greet_user": {"config": {"name": "Arthur", "username": "arthur"}}
                }
            }
        )
    assert not executed
